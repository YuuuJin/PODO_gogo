...
//                //sharedData->STATE_COMMAND = TCMD_WHEEL_POS_CHANGE_WH2WALK_DONE;
            ...
            else if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[0] == 11){
                GotoWalkReadyPos_HB_inv();
                usleep(4000*1000);
                //sharedData->STATE_COMMAND = TCMD_WALKREADY_DONE;
            }
            else if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[0] == 12){
                //Right stance
                GotoWalkReadyPos_OneLeg(-1);
                usleep(4000*1000);
                //sharedData->STATE_COMMAND = TCMD_WALKREADY_DONE;
            }
            else if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[0] == 13){
                //Left stance
                GotoWalkReadyPos_OneLeg(1);
                usleep(4000*1000);
                //sharedData->STATE_COMMAND = TCMD_WALKREADY_DONE;
...
//    joint->SetMoveJoint(RSR, 0.0, postime, MOVE_ABSOLUTE);
//    joint->SetMoveJoint(RSY, 0.0, postime, MOVE_ABSOLUTE);
//    joint->SetMoveJoint(REB, -10, postime, MOVE_ABSOLUTE);
//    joint->SetMoveJoint(RWY, 0.0, postime, MOVE_ABSOLUTE);
...
//    joint->SetMoveJoint(LSR, -0.0, postime, MOVE_ABSOLUTE);
//    joint->SetMoveJoint(LSY, 0.0, postime, MOVE_ABSOLUTE);
//    joint->SetMoveJoint(LEB, -10.0, postime, MOVE_ABSOLUTE);
//    joint->SetMoveJoint(LWY, 0.0, postime, MOVE_ABSOLUTE);
//    joint->SetMoveJoint(LWP, 0.0, postime, MOVE_ABSOLUTE);
    ...
    des_pCOM_3x1[0] = userData->WalkReadyCOM[0] = 0.0;//0.0237f;
    des_pCOM_3x1[1] = userData->WalkReadyCOM[1] = 0.0;
    des_pCOM_3x1[2] = userData->WalkReadyCOM[2] = 0.83;//0.77;// + 0.11;//(61.8kg:0.74)//59//0.8;//71kg
...
//                        , vec3(des_pLF_3x1), quat(des_qLF_4x1));
    ...
    LJ = HB_INV.IK_COM(vec3(des_pCOM_3x1),quat(des_qPEL_4x1),des_pRF_new, quat(des_qRF_4x1),vec3(des_pLF_3x1), quat(des_qLF_4x1));
   ...
   double total_mass;
   double total_lower_mass;
   double m_rleg = HB_INV.m_rhy+HB_INV.m_rhr+HB_INV.m_rhp+HB_INV.m_rkn+HB_INV.m_rap+HB_INV.m_rar;
   double m_lleg = HB_INV.m_lhy+HB_INV.m_lhr+HB_INV.m_lhp+HB_INV.m_lkn+HB_INV.m_lap+HB_INV.m_lar;
   total_lower_mass = m_rleg + m_lleg + HB_INV.m_pel;

   double m_rarm = HB_INV.m_rsp+HB_INV.m_rsr+HB_INV.m_rsy+HB_INV.m_reb+HB_INV.m_rwy+HB_INV.m_rwp+HB_INV.m_rf1;
   double m_larm = HB_INV.m_lsp+HB_INV.m_lsr+HB_INV.m_lsy+HB_INV.m_leb+HB_INV.m_lwy+HB_INV.m_rwp+HB_INV.m_lf1;
   total_mass = total_lower_mass + m_rarm + m_larm + HB_INV.m_torso;
...
void GotoWalkReadyPos_OneLeg(int left_or_right){
    ...
    if(left_or_right == -1){
        ...
        des_pLF_3x1[0] = 0.00;
        des_pLF_3x1[1] = (HB_INV.P2H)*2;
        des_pLF_3x1[2] = 0.01;//0.05;//0.0;
    ...
    if(left_or_right == 1){
        ...
        des_pRF_3x1[0] = 0.0;
        des_pRF_3x1[1] = -(HB_INV.P2H)*2;
        des_pRF_3x1[2] = 0.01;//0.05;//0.0;
...
//                        , vec3(des_pLF_3x1), quat(des_qLF_4x1));
    ...
    LJ = HB_INV.IK_COM(vec3(des_pCOM_3x1),quat(des_qPEL_4x1),vec3(des_pRF_3x1), quat(des_qRF_4x1),vec3(des_pLF_3x1), quat(des_qLF_4x1));
