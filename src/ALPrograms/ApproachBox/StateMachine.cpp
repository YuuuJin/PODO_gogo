...
_footprint_info     last_target_foot;
int                last_moving_leg = MOVING_RIGHT;
int moving_leg_target0 = MOVING_RIGHT;
...
MOVING_LEG check_moving_leg(int start_index)
{
    ...
    {
        if(FLAG_WalkingJoy == true)
        {
            if(moving_leg_target0 == MOVING_RIGHT)
                return MOVING_LEFT;
            else
                return MOVING_RIGHT;
        } else
...
void update_window_state(long w_cnt)
{
    ...
    for(int i=0; i<WIN_NUM; i++)
    {
        ...
        for(int j=0; j<target_foot_filled; j++)
        {
            ...
            {// In the DSP time ---
                switch(target_foot[j].movingleg = check_moving_leg(j-1))
        ...
        for(int j=0; j<target_foot_filled; j++)
        {
            ...
            {// In the DSP time ---
                ...
                case MOVING_RIGHT:
                {
                    if(target_foot[j].info == FOOTINFO_FIRST_STEP)
                    {// first step
                        if(cnt_2_time(w_cnt) <= (tempTime-DSP_TIME))
                        {// stay
                            window[i].zmp.x = trajectory_5th(curTime, temp_cur->time.dsp_time - DSP_TIME,
                                                            (temp_pre->footprint.rfoot[0] + temp_pre->footprint.lfoot[0])/2.0,
                                                            (temp_pre->footprint.rfoot[0] + temp_pre->footprint.lfoot[0])/2.0);
                            window[i].zmp.y = trajectory_5th(curTime, temp_cur->time.dsp_time - DSP_TIME,
                                                            (temp_pre->footprint.rfoot[1] + temp_pre->footprint.lfoot[1])/2.0,
                                                            (temp_pre->footprint.rfoot[1] + temp_pre->footprint.lfoot[1])/2.0);
                        } else
                        {// move
                            window[i].zmp.x = trajectory_5th(curTime - (tempTime-DSP_TIME), DSP_TIME,
                                                            (temp_pre->footprint.rfoot[0] + temp_pre->footprint.lfoot[0])/2.0,
                                                            temp_cur->footprint.lfoot[0]);
                            window[i].zmp.y = trajectory_5th(curTime-(tempTime-DSP_TIME), DSP_TIME,
                                                            (temp_pre->footprint.rfoot[1] + temp_pre->footprint.lfoot[1])/2.0,
                ...
                if(target_foot[j].info == FOOTINFO_LAST_STEP)
                {
                    ...
                    {// last moving leg is right
                        if(curTime < DSP_TIME)
                        {// move
                            window[i].zmp.x = trajectory_5th(curTime, DSP_TIME,
                                                            temp_cur->footprint.lfoot[0],
                                                            (temp_cur->footprint.lfoot[0] + temp_cur->footprint.rfoot[0])/2.0);
                            window[i].zmp.y = trajectory_5th(curTime, DSP_TIME,
                                                            temp_cur->footprint.lfoot[1],
...
//                        window[i].right_foot_ref.x_vel = trajectory_onecos_vel(curTime, temp_cur->time.ssp_time*0.2/5.,
//                                                                temp_pre->footprint.rfoot[0],
//                                                                temp_pre->footprint.rfoot[0]);


                    }else if(curTime > temp_cur->time.ssp_time*0.2/5. && curTime <= temp_cur->time.ssp_time*4.0/5.)
                    {
                        // leg down
                        window[i].right_foot_ref.x = trajectory_5th(curTime - temp_cur->time.ssp_time*0.2/5., temp_cur->time.ssp_time*3.8/5.,
                                                                temp_pre->footprint.rfoot[0] ,
                                                                temp_cur->footprint.rfoot[0]);

//                        window[i].right_foot_ref.x_vel = trajectory_onecos_vel(curTime - temp_cur->time.ssp_time*0.2/5., temp_cur->time.ssp_time*3.8/5.,
//                                                                temp_pre->footprint.rfoot[0] ,
//                                                                temp_cur->footprint.rfoot[0]);
...
//                        window[i].right_foot_ref.x_vel = 0.;//temp_cur->footprint.rfoot[0];//
                    ...
                    {
                        // leg down
                        window[i].right_foot_ref.y = trajectory_5th(curTime - temp_cur->time.ssp_time*0.2/5., temp_cur->time.ssp_time*3.8/5.,
                                                                temp_pre->footprint.rfoot[1] ,
                                                                temp_cur->footprint.rfoot[1]);
                    }else
                    {
    ...
    //                                                                temp_cur->footprint.lfoot[0]);
                ...
                if(curTime <= temp_cur->time.ssp_time*1.7/5.){
                    // leg up
                    window[i].right_foot_ref.z = trajectory_onecos(curTime, temp_cur->time.ssp_time*1.7/5.,
                                                            temp_pre->footprint.rfoot[2],
                                                            temp_pre->footprint.rfoot[2]+ height);
                }else{
                    // leg down
                    window[i].right_foot_ref.z = trajectory_onecos(curTime - temp_cur->time.ssp_time*1.7/5., temp_cur->time.ssp_time*3.3/5.,
                                                            temp_pre->footprint.rfoot[2]+ height,
                                                            temp_cur->footprint.rfoot[2]);
                }
...
//                        window[i].left_foot_ref.x_vel = trajectory_onecos_vel(curTime, temp_cur->time.ssp_time*0.2/5.,
//                                                                temp_pre->footprint.lfoot[0],
//                                                                temp_pre->footprint.lfoot[0]);
                    ...
                    {
                        // leg down
                        window[i].left_foot_ref.x = trajectory_5th(curTime - temp_cur->time.ssp_time*0.2/5., temp_cur->time.ssp_time*3.8/5.,
                                                                temp_pre->footprint.lfoot[0] ,
                                                                temp_cur->footprint.lfoot[0]);

//                        window[i].left_foot_ref.x_vel = trajectory_onecos_vel(curTime - temp_cur->time.ssp_time*0.2/5., temp_cur->time.ssp_time*3.8/5.,
//                                                                temp_pre->footprint.lfoot[0] ,
//                                                                temp_cur->footprint.lfoot[0]);
...
//                        window[i].left_foot_ref.x_vel = 0.;//
                    ...
                    {
                        // leg down
                        window[i].left_foot_ref.y = trajectory_5th(curTime - temp_cur->time.ssp_time*0.2/5., temp_cur->time.ssp_time*3.8/5.,
                                                                temp_pre->footprint.lfoot[1] ,
                                                                temp_cur->footprint.lfoot[1]);
                    }else
                    {
...
//                                                                temp_cur->footprint.lfoot[0]);
                    ...
                    if(curTime <= temp_cur->time.ssp_time*1.7/5.){
                        // leg up
                        window[i].left_foot_ref.z = trajectory_onecos(curTime, temp_cur->time.ssp_time*1.7/5.,
                                                                temp_pre->footprint.lfoot[2],
                                                                temp_pre->footprint.lfoot[2]+ height);
                    }else{
                        // leg down
                        window[i].left_foot_ref.z = trajectory_onecos(curTime - temp_cur->time.ssp_time*1.7/5., temp_cur->time.ssp_time*3.3/5.,
                                                                temp_pre->footprint.lfoot[2]+ height,
                                                                temp_cur->footprint.lfoot[2]);
                    }
...
//                    window[i].right_foot_ref.x_vel = 0.;
//                    window[i].left_foot_ref.x_vel = 0.;
        ...
        for(int j=0; j<target_foot_filled; j++)
        {
            ...
            {// In the SSP time ---
                ...
                case MOVING_RIGHT:
                {
                    ...
                    if(curTime <= temp_cur->time.ssp_time*1.7/5.0)
                    {// leg up
                        window[i].right_foot_ref.z = trajectory_5th(curTime, temp_cur->time.ssp_time*1.7/5.0,
                                                                temp_pre->footprint.rfoot[2],
                                                                temp_pre->footprint.rfoot[2] + height);
                        window[i].right_foot_ref.y = trajectory_5th(curTime, temp_cur->time.ssp_time*1.7/5.0,
                                                                    temp_pre->footprint.rfoot[1],
                                                                    - Legoffset);
                    } else
                    {// leg down
...
//                        }
                        ...
                        window[i].right_foot_ref.z = trajectory_5th(curTime - temp_cur->time.ssp_time*1.7/5.0, temp_cur->time.ssp_time*3.3/5.0,
                                                                temp_pre->footprint.rfoot[2] + height,
                                                                temp_pre->footprint.rfoot[2]);
                        window[i].right_foot_ref.y = trajectory_5th(curTime - temp_cur->time.ssp_time*1.7/5.0, temp_cur->time.ssp_time*3.3/5.0,
                                                                    -Legoffset,
                                                                    temp_cur->footprint.rfoot[1]);
                    }
                    window[i].left_foot_ref.x = prev_foot.footprint.lfoot[0];
                ...
                case MOVING_LEFT:
                {
                    ...
                    if(curTime <= temp_cur->time.ssp_time*1.7/5.0)
                    {// leg up
                        window[i].left_foot_ref.z = trajectory_5th(curTime, temp_cur->time.ssp_time*1.7/5.0,
                                                                temp_pre->footprint.lfoot[2],
                                                                temp_pre->footprint.lfoot[2]+ height);
                        window[i].left_foot_ref.y = trajectory_5th(curTime, temp_cur->time.ssp_time*1.7/5.0,
                                                                    temp_pre->footprint.lfoot[1],
                                                                    + Legoffset);
                    } else
                    {// leg down
                        window[i].left_foot_ref.z = trajectory_5th(curTime - temp_cur->time.ssp_time*1.7/5.0, temp_cur->time.ssp_time*3.3/5.0,
                                                                temp_pre->footprint.lfoot[2]+ height,
                                                                temp_pre->footprint.lfoot[2]);
                        window[i].left_foot_ref.y = trajectory_5th(curTime - temp_cur->time.ssp_time*1.7/5.0, temp_cur->time.ssp_time*3.3/5.0,
                                                                    + Legoffset,
                                                                    temp_cur->footprint.lfoot[1] );
                    }
                    window[i].right_foot_ref.x = prev_foot.footprint.rfoot[0];
...
void update_window()
{
    ...
    if(target_foot[0].time.dsp_time + target_foot[0].time.ssp_time < cnt_2_time(window_cnt))
    {
        ...
        if(FLAG_WalkingJoy == true)
        {
            moving_leg_target0 = check_moving_leg(-1);
            for(int i=1; i<TARGET_FOOT_NUM; i++)
            {
        ...
        {// Move the target_foot sequence one space forward and fill last target_foot from short_foot[]
            ...
            } else if(new_footprint.mode == WALKING_NORMAL && FLAG_BacktoNormalWalk == -1)
            {
                printf("final single walk\n");
                new_footprint.time.ssp_time = 3.0;
                new_footprint.time.dsp_time = 3.0;
                FLAG_BacktoNormalWalk = 0;
            }
            memcpy(&last_short_foot, &new_footprint, sizeof(_footprint_info));
...
////        }
        last_moving_leg = check_moving_leg(-1);
    ...
    if(target_foot[0].mode == WALKING_SINGLELOG || target_foot[0].mode == WALKING_SINGLELOG_FINAL)
        FLAG_SingleLog = true;
    /* make Trajectory of foot pos,ori */
    if(target_foot[0].mode == WALKING_SINGLELOG || target_foot[0].mode == WALKING_CHANGE || target_foot[0].mode == WALKING_SINGLELOG_FINAL)
    {
