...
void FPG_TEST(int dir,int step_num,double step_length,double step_angle,double step_offset,double LPEL2PEL)
{
    ...
    if(step_angle>15.0)         step_angle = 15.0;
    else if(step_angle<-15.0)   step_angle = -15.0;
    ...
    if(dir == APPROACHBOX_RIGHT_WALKING)    right_left = 0;
    else if(dir == APPROACHBOX_LEFT_WALKING)right_left  = 1;
    ...
    while(pull_short_foot(dummyfoot)){// remove old foot printf;
        ;
    }
    ...
    if(dir != APPROACHBOX_FORWARD_WALKING)
    {
        ...
        if(test_short_foot_num < TARGET_FOOT_NUM)
        {
            test_short_foot_num = TARGET_FOOT_NUM;
        }

    }

    if(step_offset<0.25)    step_offset = 0.25;
...
void FPG_SINGLELOG(int step_num,double step_length,double LPEL2PEL)
{
    ...
    /* Decide the foot to move first. zero is right */
    int right_left = 0;

    if(step_num < 2) step_num = 2;
    ...
    _footprint_info tempFoot;
    tempFoot.footprint.rfoot[0] = FK_pRFoot_3x1[0];
    tempFoot.footprint.rfoot[1] = FK_pRFoot_3x1[1];
    tempFoot.footprint.rfoot[2] = FK_pRFoot_3x1[2];
    ...
    tempFoot.footprint.lfoot[0] = FK_pLFoot_3x1[0];
    tempFoot.footprint.lfoot[1] = FK_pLFoot_3x1[1];
    tempFoot.footprint.lfoot[2] = FK_pLFoot_3x1[2];
    ...
    int test_short_foot_num = step_num;
    double step_width = para.SINGLELOG_WIDTH;
    double sin_dsp_time_init = para.SINGLELOG_DSP_INIT;
    double sin_dsp_time = para.SINGLELOG_DSP;
    double sin_ssp_time = para.SINGLELOG_SSP;


    for(int i=0; i<test_short_foot_num+2; i++)
    {
        if(i == 0)
        {// First Foot Print (Generally Right foot first)
            ...
            tempFoot.time.dsp_time = sin_dsp_time_init;
            tempFoot.time.ssp_time = sin_ssp_time;
            tempFoot.mode = WALKING_SINGLELOG;
        ...
        {// Third Foot Print from the LAST.
            ...
            tempFoot.time.dsp_time = sin_dsp_time;
            tempFoot.time.ssp_time = sin_ssp_time;
            tempFoot.mode = WALKING_SINGLELOG_FINAL;

        }else if(i == test_short_foot_num)
...
void FPG_MANUAL()
{
    ...
    {
        tempFoot.footprint = MnFootPrint.FootPrint[i];
        tempFoot.time.dsp_time = MnFootPrint.DSPtime[i];
        tempFoot.time.ssp_time = MnFootPrint.SSPtime[i];

        tempFoot.mode = WALKING_STEPPING_STONE;

        printf("Push Short Foot : %f, %f, %f, %f\n",tempFoot.footprint.rfoot[0],tempFoot.footprint.rfoot[1],tempFoot.footprint.lfoot[0],tempFoot.footprint.lfoot[1]);
        printf("PushTime : %f, %f\n",tempFoot.time.dsp_time, tempFoot.time.ssp_time);
        push_short_foot(tempFoot);
    }
}
