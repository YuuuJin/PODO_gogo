...
//    double A2F;//not sure
    ...
    HB_inverse()
    {
        ...
        m_rhy = 0.928;
        m_rhr = 1.590;
        m_rhp = 7.876-2.5;//3.888;//4.888;
        m_rkn = 3.701-1;
        m_rap = 1.466;
        m_rar = 1.907;//RAR+RAFT+FOOT
        m_rleg = m_rhy + m_rhr + m_rhp + m_rkn + m_rap + m_rar; //9.601

        m_lhy = 0.928;
        m_lhr = 1.590;
        m_lhp = 7.876-2.5;//3.888;//4.888;
        m_lkn = 3.701-1;
        m_lap = 1.466;
        m_lar = 1.907;//RAR+RAFT+FOOT
...
//        m_lf1 = 0.001;
        ...
        m_lwy = 1.47;
        m_lwp = 0.33916;
        m_lf1 = 0.931705; // +finger

        m_torso = 24.98;
        //no waste joint version, this pelvis contains torso mass
        m_pel = 18.0 + (m_rsp + m_rsr + m_rsy + m_reb + m_rwy + m_rwp + m_rf1)*2 + m_torso + 2.0;//
        ...
        P2SC = 0.456; // pelvis to shoulder center
        SC2S = 0.249;//0.2145; // shoulder center to shoulder
        UARM = 0.35;//0.17914;
        LARM = 0.34;
        OFFELB = 0.035;
...
//                +m_lsp+m_lsr+m_lsy+m_leb+m_lwy+m_lwp+m_lf1;
        ...
        //centerofmass_lb
        c_rhy = vec3(-0.009, 0.0, 0.0624);
        c_rhr = vec3(0.021225, -0.00221, 0.007);
        c_rhp = vec3(0.0030,-0.02021,-0.2266);
        c_rkn = vec3(0.005,-0.0202,-0.064);
        c_rap = vec3(0.0172034, -0.002249, -0.00449);
        c_rar = vec3(-0.009, -0.001, -0.0217225);

        c_lhy = vec3(-0.009, 0.0, 0.0624);
        c_lhr = vec3(0.021225, 0.00221, 0.007);
        c_lhp = vec3(0.0030, 0.02021,-0.2266);
        c_lkn = vec3(0.005, 0.0202,-0.064);
        c_lap = vec3(0.0172034, 0.002249, -0.00449);
        c_lar = vec3(-0.009, 0.001, -0.0217225);
...
//                [ -sin(q3 + q4 + q5), 0, cos(q3 + q4 + q5)]
    ...
    LegJoints IK_COM(vec3 pCOM, quat qPel, vec3 pRF, quat qRF, vec3 pLF, quat qLF)
    {
        ...
        {
            LJ = IK_pel_drc(pPel,qPel,pRF,qRF,pLF,qLF);//loop loop loop
