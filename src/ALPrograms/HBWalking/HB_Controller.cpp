...
void HB_PreviewWalk::DampingControl3(){  // FT and IMU are used
    ...
    double Ks = 4376.8;// NRL DRC Hubo
    double C = 82.51;// NRL DRC Hubo
    double m = 81;

    double Ksx = 3005.9401;
    double Cx = 105.3596;
...
//    vec3 Kc = vec3(k1, k2, 0);
    ...
    vec3 Kc = vec3(-0.3306,    0.1070, 0); // pole placement [-6+0.001i -6-0.001i];
    vec3 Kcx = vec3(-0.1072,    0.1497,0); //pole placement [-4+0.001i -4-0.001i];
...
//                   -0.1255, -1.380, 0.0702,
//                   0,0,0);
    mat3 Ke = mat3(2.7378,    0.5162,   -0.2032,
                   -3.5702,   -1.0159,    3.9010,
                   0,0,0);

//    mat3 Kex = mat3(9.2710 ,   0.0714,   -0.0667,
//                    -5.6173,   -0.1335,    1.1961,
//                     0,0,0);
    mat3 Kex = mat3( 9.6098,    0.2659,   -0.0672,
                     -3.2377,   -0.3360,    1.4316,
                    0,0,0);
...
//    dCOM_error_local = global2local_vec(qRF_ref, qLF_ref, dCOM_m_diff - dCOM_LIPM);  // dCOM error local
    ...
    if(COM_damping_con_local.y > 0.07) COM_damping_con_local.y = 0.07;
    if(COM_damping_con_local.y < -0.07) COM_damping_con_local.y = -0.07;
    if(COM_damping_con_local.x > 0.05) COM_damping_con_local.x = 0.05;
    if(COM_damping_con_local.x < -0.05) COM_damping_con_local.x = -0.05;
...
vec4 HB_PreviewWalk::Ankle_Torque_from_cZMP(vec3 _global_cZMP, vec3 _global_COM, quat _global_qRF, quat _global_qLF,
    ...
    vec3 local_COM = global2local_point(_global_qRF, _global_qLF,  _global_pRF, _global_pLF, _global_COM);
    ...
    double _Xoffset = 0;
    double _Yoffset = +0.012;

    // get support polygon data for local frame
    vec3 LF_lt1 = vec3((FootLength+_Xoffset)/2, (FootWidth +_Yoffset)/2  + 0.01, 0);
    vec3 LF_rt2 = vec3((FootLength+_Xoffset)/2, -(FootWidth +_Yoffset)/2 + 0.01 , 0);
    vec3 LF_rb3 = vec3(-(FootLength+_Xoffset)/2, -(FootWidth +_Yoffset)/2 + 0.01, 0);
    vec3 LF_lb4 = vec3(-(FootLength+_Xoffset)/2, (FootWidth +_Yoffset)/2  + 0.01, 0);

    vec3 RF_lt1 = vec3((FootLength+_Xoffset)/2, (FootWidth +_Yoffset)/2 - 0.01 , 0);
    vec3 RF_rt2 = vec3((FootLength+_Xoffset)/2, -(FootWidth +_Yoffset)/2 - 0.01, 0);
    vec3 RF_rb3 = vec3(-(FootLength+_Xoffset)/2, -(FootWidth +_Yoffset)/2 - 0.01, 0);
    vec3 RF_lb4 = vec3(-(FootLength+_Xoffset)/2, (FootWidth +_Yoffset)/2 - 0.01, 0);
    ...
    Point P_local_cZMP = {local_COM.x, local_COM.y};
    ...
    if(_F_RF.z > 30 && _F_LF.z > 30){  // When both feet are on the ground (DSP)
...
void HB_PreviewWalk::AnkleTorqueController_pos(double _RAR_T_ref, double _RAP_T_ref, double _LAR_T_ref, double _LAP_T_ref,
    ...
    //// variable gains
    double gain_roll_max = -0.4;//-0.5;
    double gain_roll_min = -0.005;//plat floor //-0.006;uneven//-0.035;

    double gain_pitch_max = -0.4;//-0.5;//-0.5;
    double gain_pitch_min = -0.004;//plat floor//-0.008;unenven//-0.045;
...
//    double Fc_Max = 3.5;
//    double Fc_min = 1.0;
//    double Fc = (1.0 - 3.5)/200*_F_RF_filtered_new.z + 3.5;
...
//    RF_variable_gain_pitch = gain_pitch_air*1/(1 + _F_RF_filtered_new.z/decrease_factor_pitch);
    ...
    RF_variable_gain_roll = (gain_roll_min - gain_roll_max)/200*_F_RF_filtered_new.z + gain_roll_max;
    RF_variable_gain_pitch = (gain_pitch_min - gain_pitch_max)/200*_F_RF_filtered_new.z + gain_pitch_max;
    ...
    RF_Arial_return_factor = 0.1;//0.1*(10 + _F_RF_filtered.z)/10;
...
//    //cout<<"roll gain : "<<LF_varible_gain_roll<<"  pitch gain : "<<LF_variable_gain_pitch<<endl;
    ...
    LF_variable_gain_roll = (gain_roll_min - gain_roll_max)/200*_F_LF_filtered_new.z + gain_roll_max;
    LF_variable_gain_pitch = (gain_pitch_min - gain_pitch_max)/200*_F_LF_filtered_new.z + gain_pitch_max;
    ...
    LF_Arial_return_factor = 0.1;//0.1*(10 + _F_LF_filtered.z)/10;
    ...
    double Max_ankle_vel_roll = 1.2;
    double Max_ankle_vel_pitch = 1.0;

    if(dRF_angle_ctrl.x > Max_ankle_vel_roll) dRF_angle_ctrl.x = Max_ankle_vel_roll;
    if(dRF_angle_ctrl.x < -Max_ankle_vel_roll) dRF_angle_ctrl.x = -Max_ankle_vel_roll;

    if(dRF_angle_ctrl.y > Max_ankle_vel_pitch) dRF_angle_ctrl.y = Max_ankle_vel_pitch;
    if(dRF_angle_ctrl.y < -Max_ankle_vel_pitch) dRF_angle_ctrl.y = -Max_ankle_vel_pitch;

    if(dLF_angle_ctrl.x > Max_ankle_vel_roll) dLF_angle_ctrl.x = Max_ankle_vel_roll;
    if(dLF_angle_ctrl.x < -Max_ankle_vel_roll) dLF_angle_ctrl.x = -Max_ankle_vel_roll;

    if(dLF_angle_ctrl.y > Max_ankle_vel_pitch) dLF_angle_ctrl.y = Max_ankle_vel_pitch;
    if(dLF_angle_ctrl.y < -Max_ankle_vel_pitch) dLF_angle_ctrl.y = -Max_ankle_vel_pitch;
    ...
    if(Pos_Ankle_torque_control_flag == true){
        ...
        double angle_limit = 40; //deg
        double angle_limit_rol = 30;
...
//        DSP_time_flag = false;
    ...
    vec3 LF_lt1 = vec3(FootLength/2, FootWidth/2+0.01, 0);
    vec3 LF_rt2 = vec3(FootLength/2, -FootWidth/2+0.01, 0);
    vec3 LF_rb3 = vec3(-FootLength/2, -FootWidth/2+0.01, 0);
    vec3 LF_lb4 = vec3(-FootLength/2, FootWidth/2+0.01, 0);

    vec3 RF_lt1 = vec3(FootLength/2, FootWidth/2-0.01, 0);
    vec3 RF_rt2 = vec3(FootLength/2, -FootWidth/2-0.01, 0);
    vec3 RF_rb3 = vec3(-FootLength/2, -FootWidth/2-0.01, 0);
    vec3 RF_lb4 = vec3(-FootLength/2, FootWidth/2-0.01, 0);
...
//    Alpha_dsp = (A + 1.0)/2.0;
    ...
    if(step_phase <= 1 || step_phase > N_step - 2){
        dsp_ctrl_gain = 0.00002; //0.0008
        dsp_tilt_gain = 5.5;//0.9
    }
    else{
        dsp_ctrl_gain = 0.0005; //0.0012
        dsp_tilt_gain = 5.0; //10
    }
...
//    double alpha_at = Alpha_AT;

//    if(DSP_force_flag == true || DSP_time_flag == true){
//        if(alpha_at < 0.3) alpha_at = 0.3;
//        if(alpha_at > 0.7) alpha_at = 0.7;
//    }

//    RF_Fz_ref = alpha_at*Fz_total;
//    LF_Fz_ref = (1.0 - alpha_at)*Fz_total;
...
//    dsp_tilt_gain = 0;
    ...
    if(DSP_time_flag == true || DSP_force_flag == true){
        ...
        dz_ctrl = dsp_ctrl_gain*Fz_diff_error + dsp_tilt_gain/1*COM_e_imu_local.y
                    + L*dsp_tilt_gain*COM_e_imu_local.x;

//        if((COM_e_imu_local.y > 0.03 && RS.F_RF.z < 60) || (COM_e_imu_local.y < -0.03 && RS.F_LF.z < 60)){
...
//        Pelv_roll_vel_ref = 0*COM_e_imu_local.y - Pelv_roll_ref/0.5;
        ...
        dz_ctrl = dsp_ctrl_gain/2.0*Fz_diff_error + dsp_tilt_gain/4.0*COM_e_imu_local.y
                    +0.0*L*dsp_tilt_gain*COM_e_imu_local.x - z_ctrl/0.2;
...
//        if((COM_e_imu_local.y > 0.04 && RS.F_RF.z < 60) || (COM_e_imu_local.y < -0.04 && RS.F_LF.z < 60)){
//            dz_ctrl = dsp_ctrl_gain/4.0*Fz_diff_error - dsp_tilt_gain/9*COM_e_imu_local.y
...
void HB_PreviewWalk::Calc_del_pos_from_Joystick(int _RJOG_RL, int _RJOG_UD, int _LJOG_RL, int _LJOG_UD, double _des_t_step){
    ...
    //version 1
    double del_pos_Max_x = 0.25;
    double del_pos_min_x = 0.25;

    double del_pos_Max_y = 0.15;
    double del_pos_Max_yaw = 10; //deg
    ...
    if(_des_t_step <= 0.7) _des_t_step = 0.7;
...
//    if(Alpha_landing <= 0.0) Alpha_landing = 0.0;
    ...
    double Fz_total = 810.0; //32kg
    double RF_Threshold = Alpha_dsp*Fz_total + 30;
    double LF_Threshold = (1.0 - Alpha_dsp)*Fz_total + 30;

    if(RF_Threshold < 50.0) RF_Threshold = 50.0;
    if(LF_Threshold < 50.0) LF_Threshold = 50.0;
