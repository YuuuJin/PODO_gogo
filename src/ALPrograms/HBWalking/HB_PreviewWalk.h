...
//    Gazelle_Kine GK;
    ...
    vec4 Ankle_Torque_from_cZMP(vec3 _global_cZMP, vec3 _global_COM, quat _global_qRF, quat _global_qLF,
...
//        RF_angle_ctrl = vec3();
        ...
        for(int i = 0; i < N_step + 3 ;i++){
            if(i == 0){
                ...
                SD_temp.Fpos = zmp_ini + vec3(0,+0.00,0); //for fast start
                SD_temp.Fquat = quat();
                SD_temp.yaw_rad = 0;
...
//                SDB.push_back(SD_temp);
    ...
    int PreviewGainLoad(double _zc){
        ...
        ifstream file;
        file.open("../PreviewGain_5s.txt",ios::in);
