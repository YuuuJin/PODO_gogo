...
//    Using damping con + imu error
    ...
    // Using only imu error
    vec3 temp_cZMP_TC_local = global2local_point(qRF_ref, qLF_ref, pRF_ref, pLF_ref, p_ref[0]) + (1 + 30.0/w)*(COM_e_imu_local + 0.83*dCOM_e_imu_local/w);
    ...
    cZMP_TC_proj = zmpProjectionToSP_offset(cZMP_TC_global, pRF_ref, pLF_ref, qRF_ref, qLF_ref, RS.F_RF, RS.F_LF, +0.02, +0.02);
...
//        CP_error_lf.y = CP_error_lf.y*0.9;
        ...
        // CP error clampin
        if(CP_error_lf.y > 0.10) CP_error_lf.y = 0.10;
        if(CP_error_lf.y < -0.10) CP_error_lf.y = -0.10;

//        if(CP_error_lf.x > 0.13) CP_error_lf.x = 0.13;
//        if(CP_error_lf.x < -0.13) CP_error_lf.x = -0.13;

        if(CP_error_lf.x > 0.15) CP_error_lf.x = 0.15;
        if(CP_error_lf.x < -0.15) CP_error_lf.x = -0.15;
        ...
        double dT_cZMP_y = 0.65;// - t_now;//t_step - t_now;
        ...
        cZMP_SA_lf = zmpProjectionTo_StanceFoot(RFoot, cZMP_SA_lf_before_saturate,-0.11, -0.08);
...
//        CP_error_rf.y = CP_error_rf.y*0.9;
        ...
        //CP error clamping
        if(CP_error_rf.y > 0.10) CP_error_rf.y = 0.10;
        if(CP_error_rf.y < -0.10) CP_error_rf.y = -0.10;

        if(CP_error_rf.x > 0.15) CP_error_rf.x = 0.15;
        if(CP_error_rf.x < -0.15) CP_error_rf.x = -0.15;
        ...
        // cZMP saturation into left foot
        cZMP_SA_rf = zmpProjectionTo_StanceFoot(LFoot, cZMP_SA_rf_before_saturate,-0.11, -0.08);
...
//                cout<<"Lf landing x: "<<pLF_landing.x<<"  pLF_landing.y: "<<pLF_landing.y<<endl;
    ...
    AnkleTorque_ref = Ankle_Torque_from_cZMP(cZMP_TC_proj, COM_m, qRF_ref, qLF_ref, pRF_ref, pLF_ref, RS.F_RF, RS.F_LF);
...
//    LF_landing_angle_gain_pitch = 12*50/(50 + RS.F_LF.z);
    ...
    if(Joint_deflection_compensation_flag == true){
        if(SDB[step_phase].swingFoot == RFoot){
            ...
            L_roll_compen_deg = alpha*L_roll_compen_deg + (1-alpha)*HB_sign(ZMP_local.y)*0.1*10;
        ...
        if(SDB[step_phase].swingFoot == LFoot){
            ...
            // hip roll
            double alpha = 1/(1 + 2*PI*dt*3);
            R_roll_compen_deg = alpha*R_roll_compen_deg + (1-alpha)*HB_sign(ZMP_local.y)*0.1*10;
...
void HB_PreviewWalk::MeasurementInput(RobotStates _RST){
    RS = _RST;
...
//    CP_m = RS.CSP.pCOM + RS.CSV.dpCOM/w;  CP_m.z = 0;
    ...
    COM_e_imu_local.x = alpha_COMe_imu*COM_e_imu_local.x + (1-alpha_COMe_imu)*G_R_g_pitroll_rpy.p*0.82;
    COM_e_imu_local.y = alpha_COMe_imu*COM_e_imu_local.y + (1-alpha_COMe_imu)*(-G_R_g_pitroll_rpy.r*0.82);
...
//    COM_e_imu_local.y = alpha_COMe_imu*COM_e_imu_local.y + (1-alpha_COMe_imu)*(-RS.IMUangle.x*0.72);
    ...
    dCOM_e_imu_local.x = alpha_dCOMe_imu*dCOM_e_imu_local.x + (1-alpha_dCOMe_imu)*RS.IMULocalW.y*0.82;
    dCOM_e_imu_local.y = alpha_dCOMe_imu*dCOM_e_imu_local.y + (1-alpha_dCOMe_imu)*(-RS.IMULocalW.x*0.82);
...
//    // get Imu rot matrix w.r.t global without yaw (only pitch and roll)
//    mat3 rot_pitroll_wrtpelv = mat3(cos(_IMUangle.y),sin(_IMUangle.y*D2Rf)*sin(_IMUangle.x), sin(_IMUangle.y)*cos(_IMUangle.x),
//                          0, cos(_IMUangle.x), -sin(_IMUangle.x),
//                          -sin(_IMUangle.y), cos(_IMUangle.y)*sin(_IMUangle.x), cos(_IMUangle.y)*cos(_IMUangle.x));

//    ////Real robot tilt : Rotation matrix between Real global frame(G) and WBIK frame(g) (without yaw)
//    // WBIK qPel with out yaw

//    rpy rpy_Pel = rpy(_qPel);
//    mat3 rot_Pel_pitroll = mat3(vec3(0,1,0),rpy_Pel.p)*mat3(vec3(1,0,0), rpy_Pel.r);

//    // Real tilt
//    G_R_g_pitroll = rot_pitroll_wrtpelv*inverse_HB(rot_Pel_pitroll);




//    // Measure Global COM position using FK and ZMP and Gyro
//    mat3 rot_pel_yaw = mat3(vec3(0,0,1), rpy_Pel.y);

//    _COM.z = _zc;
//    _ZMP_global.z = 0;

//    vec3 ZMP2COM = _COM - _ZMP_global;

//    vec3 COM_measure = _ZMP_global + rot_pel_yaw*G_R_g_pitroll*ZMP2COM;
